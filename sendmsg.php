<?php
	...
	*/
	if($body_msg != '') {
		...
		$fp = fopen("log.html", 'a');
		$data = 
		'<div class="alluser_msg ' .$user. "_message" . '">
			<p>'.$body_msg.'</p>
			<div class="alluser_detail ' .$user. "_message_detail" . '">
				<span class="specify_user">' .$user . '</span> - '.$day_sent.'/'.$month_sent.'/'.$year_sent.' lúc '.$hour_sent.':'.$min_sent.'
			</div>
		</div>';
		fwrite($fp, $data);
		fclose($fp);
